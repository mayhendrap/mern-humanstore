...
export const signin = async (req, res) => {
    ...
    try {
        ...
        if (!existingUser) return res.status(404).json({ message: "User tidak terdaftar." });
        
        const isPasswordCorrect = await bcrypt.compare(password, existingUser.password);
        if (!isPasswordCorrect) return res.status(400).json({ message: "Email/Password tidak valid."});
